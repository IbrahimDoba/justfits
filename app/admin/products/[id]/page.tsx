...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fetch product
        const prodResponse = await fetch(`/api/admin/products/${productId}`);
        if (!prodResponse.ok) throw new Error("Failed to load product");
        ...
        setVariants(
          product.variants.map((v: any) => ({
            ...
            sku: v.sku,
            price: v.price,
            stock: v.stockQuantity, // Schema uses stockQuantity
          }))
        );

        // Map images
        // Assuming images are objects { url: string }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        status: formData.status, // Pass status string
      };
  ...
  const handleAiImageGenerated = (imageUrl: string) => {
    // For DALL-E generated URLs, they are temporary.
    // Ideally we'd upload this to Cloudinary immediately or sends it to backend to upload.
    // But for now, let's treat it as a valid URL.
    // Note: DALL-E URLs expire in an hour.
    // If we want to persist, the `ImageUpload` or backend needs to handle "uploading" it.
    // The `ImageUpload` component accepts strings. Ideally the user downloads and re-uploads, OR we handle it.
    // For this task, let's append it.
    // In a real flow, checking if it's a temp OpenAI URL and saving it to Cloudinary is best.
    setImages([...images, imageUrl]);
    // Note to user: These expire.
  };
